...
class AccountMoveLine(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None, **kwargs):
        ...
        Eliminar del dominio el campo 'shared_to_branches' que ya no existe en v19.
        Este campo era usado por módulos de multi-branch en v18.
        Interceptamos ANTES de que el dominio sea optimizado/validado.
        """
        # Limpiar el dominio recursivamente
        domain = self._clean_domain_shared_to_branches(domain)
    ...
    def _clean_domain_shared_to_branches(self, domain):
        """
        Limpia recursivamente el dominio eliminando referencias a shared_to_branches.
        Maneja tanto listas normales como objetos Domain de Odoo.
        """
        if not domain:
            return domain
        ...
        if isinstance(domain, (list, tuple)):
            ...
            for item in domain:
                if isinstance(item, (list, tuple)) and len(item) >= 1:
                    # Si es una condición (field, operator, value)
                    if isinstance(item[0], str) and item[0] != 'shared_to_branches':
                        cleaned.append(item)
                    elif not isinstance(item[0], str):
                        # Es una lista anidada, limpiarla recursivamente
                        cleaned.append(self._clean_domain_shared_to_branches(item))
