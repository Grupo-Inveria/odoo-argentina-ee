...
class AccountJournal(models.Model):
    ...
    * A través de tags: si definimos tags, se buscan impuestos pendientes de
    ...
    def _get_tax_settlement_entry_lines_vals(self, domain=None):
        ...
        grouped_move_lines = self.env["account.move.line"]._read_group(
            domain, groupby=["account_id"], aggregates=["balance:sum", "amount_currency:sum"]
        )
        ...
        for account, group_balance_sum, group_amount_currency_sum in grouped_move_lines:
            group_balance = company_currency.round(group_balance_sum)
            ...
            if currency := account.currency_id:
                if new_vals_line["debit"] > 0.0:
                    amount_currency = (
                        group_amount_currency_sum < 0.0 and -group_amount_currency_sum or group_amount_currency_sum
                    )
                else:
                    amount_currency = (
                        group_amount_currency_sum > 0.0 and -group_amount_currency_sum or group_amount_currency_sum
    ...
    def _get_tax_settlement_lines_domain_by_tags(self):
        ...
        ODOO 19 MIGRATION FIX:
        Sobrescribimos para eliminar referencias al campo 'shared_to_branches' que viene
        del método _check_company_domain() en módulos de multi-company v18.
        """
        self.ensure_one()
        company_id = self.company_id.id
        if self.company_id.child_ids:
        ...
        # Construir el dominio base sin usar _check_company_domain()
        # que puede agregar el campo shared_to_branches que no existe en v19
        domain = [
            ("company_id", "in", company_id if isinstance(company_id, list) else [company_id]),
