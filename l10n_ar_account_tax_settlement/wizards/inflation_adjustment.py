...
class InflationAdjustment(models.TransientModel):
    ...
    def confirm(self):
        ...
        init_data = account_move_line._read_group(
            domain,
            groupby=["account_id"],
        ...
        for account, balance_sum in init_data:
            adjustment = balance_sum * initial_factor
            ...
            lines.append(
                {
                    ...
                    % (FormatAmount(balance_sum), initial_factor * 100.0),
        ...
        for period in periods:
            ...
            data = account_move_line._read_group(domain, groupby=["account_id", "date"], aggregates=["balance:sum"])
            date_from = period.get("date_from")
            for account, date, balance_sum in data:
                adjustment = balance_sum * period.get("factor")
                if self.company_id.currency_id.is_zero(adjustment):
                    continue
                else:
                    adjustment = self.company_id.currency_id.round(adjustment)
